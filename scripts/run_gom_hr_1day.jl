...
# --------------------------------------------------------------------------- #
# Output writer: save hourly fields for animation
# --------------------------------------------------------------------------- #
...
init_snapshot_file = joinpath(output_dir, "gom_hr_1day_init_snapshot_hycom.jld2")

η_init = Array(interior(ocean_model.free_surface.η))   # 2D
T_init = Array(interior(ocean_model.tracers.T))        # 3D
S_init = Array(interior(ocean_model.tracers.S))        # 3D

@info "Saving initial HYCOM-initialized snapshot to $init_snapshot_file"
jldsave(init_snapshot_file; η = η_init, T = T_init, S = S_init)
@info "Initial snapshot saved."
...
snapshot_file = joinpath(output_dir, "gom_hr_1dayssh_snapshot_hycom.jld2")
